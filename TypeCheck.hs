...
typecheck ((Function t name params stmts):ast) env = typecheck (stmts ++ ast) env -- checka även params mot typer, lägg in funktionstyp i env
typecheck ((Var name):ast) env = typecheck (ast) env
typecheck ((BinaryOp name left right):ast) env = typecheck (left:(right:ast)) env -- lägg in i env vid =
...
typecheck (expr:ast) env = "Other " ++ (show expr) ++ "\n" ++ (typecheck ast env)
